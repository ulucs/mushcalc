defmodule Mushcalc.Flames.Scores do
  ...
  def gen_score_pdf(item_type, item_level, char_eqs, method) do
    stat_combinations(item_type)
    |> expand_selection_with_tiers(item_level, item_type, method)
  ...
  def get_score_pdf_q(item_type, item_level, char_eqs, method) do
    if 0 ==
         Pdf.query(item_type, item_level, char_eqs, method)
         |> select([f], count(1))
         |> Repo.one() do
      Repo.insert_all(Pdf, gen_score_pdf(item_type, item_level, char_eqs, method))
    end
  ...
  def get_improvement_stats(item, char_eqs, method) do
    get_score_pdf_q(item.type, item.level, char_eqs, method)
